...
class FA_CheckExistsGroup:

    @staticmethod
    def check_exists(password, username, groupname):
        ...
        if fe(filename='ddbb/kdbx/' + str(username) + ".kdbx") == True:
            ...
            try:
                if db.find_groups(name=decrypt_groupname, first=True):return {'success': e(str(True)), 'exists': e(str(True))}
                else:return {'success': e(str(True)), 'exists': e(str(False))}
            except Exception as error:return {'success': e(str(False)), 'error': e(error)}
