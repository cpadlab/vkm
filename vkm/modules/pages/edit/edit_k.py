...
class FA_EditKey:
    ...
    def add_key_tovault(password, username, inputTitle, inputURL,
        selectGroup, inputUsername, inputPassword, code
    ):
        ...
        if fe(filename='ddbb/kdbx/' + str(username) + ".kdbx") == True:
            ...
            else:group  = kp.find_groups(name=f"{d(selectGroup)}", first=True);kp.save()
            
            try:kp.add_entry(destination_group=group,title=key_title,username=key_username,password=key_password,url=key_url,notes=code)
            except Exception as error:return {'success': e(str(False)), 'error': e(f"500 Internal Error. {error}")}

            if entry:kp.delete_entry(entry)
            kp.save()
